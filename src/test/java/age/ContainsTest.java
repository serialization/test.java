...
public class ContainsTest extends common.CommonTest {
    ...
    /**
     * supposed to be false because documentation says so
     */
    @Test
    ...
    public void testContainsUnknown() throws Exception {
        ...
        for (Access<? extends Obj> acc : sg1.allTypes()) {
            ...
            while (it.hasNext()) {
                Obj x = it.next();
                Assert.assertFalse(sg.contains(x));
                Assert.assertTrue(sg1.contains(x));
            }
        }
    }
    ...
    /**
     * a state should not contain a deleted instance, no matter whether the state has been flushed or not.
     */
    @Test
